...
主要介绍当今广泛使用的以开发FPGA和CPLD为主的EDA工具，及部分关于ASIC设计的EDA工具。
...
* 性能良好的FPGA/CPLD设计的HDL综合器有如下三种：
  ...
  1. Synopsys公司的DC FPGA综合器。

  2. S ynopsys公司的Synplify Pro综合器。

  3. MentorLeonardoSpectrum综合器、Precision RTL。
...
* 按照仿真电路描述级别的不同，HDL仿真器可以单独或者综合完成以下各个仿真步骤：
...
* 适配器的任务是完成目标系统在器件上的布局布线。适配，即结构综合通常都由可编程逻辑器件的厂商提供专门针对器件开发的软件来完成。这些软件可以单独或嵌入在厂商的针对自己产品的集成EDA开发环境中存在。

### 5.5 下载器（编程器）

* 将适配好的文件下载到FPGA中
* 在FPGA方面，下载器都是由FPGA厂商提供

### 5.6 EDAr软件提供商

* Synopsys（美国）
* Cadence（美国）
* MENTOR GRAPHICS（西门子，从美国收购）

### 5.7 常见的FPGA/CPLD集成开发环境

1. Intel PSG(ALTERA): Quartus 2
2. XILINX: Vivado ISE(最大的FPGA提供厂商，被AMD收购)
3. MicroSemi: Libero（美国）
4. LATTICE: Diamond、iCEcube2、ispLever（美国）

##### Quartus 2 概述

![截屏2022-09-23 下午10.26.35](Image/截屏2022-09-23 下午10.26.35.png)

![截屏2022-09-23 下午10.27.25](Image/截屏2022-09-23 下午10.27.25.png)
